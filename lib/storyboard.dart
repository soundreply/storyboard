...
abstract class Story extends StatelessWidget {
  ...
  Widget widgetListItem(Widget w, String title) => Padding(
    ...
    if (!isFullScreen) {
      return ExpansionTile(
        ...
        children: storyContent
            .map((entry) => widgetListItem(entry.widget, entry.description))
            .toList(),
      );
    } else {
      if (storyContent.length == 1) {
        return _widgetTileLauncher(storyContent[0].widget, title, context);
      } else {
        return ExpansionTile(
